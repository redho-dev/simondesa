...
<div class="row justify-content-center mt-2 mb-4">
    <div class="col-md-12 col-sm-12  ">
        <h5 class="alert alert-info">Form Edit/Update Data Penatausahaan Pajak</h5>
        <div class="x_panel">
            ...
            <div class="x_content">
                ...
                <ul class="nav nav-tabs bar_tabs" id="myTab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link {{ $jenis=='billing' ? 'active' : '' }}"
                            href="?jenis=billing&tahun={{ $tahun }}" role="tab">Tanda Terima Setoran
                            Pajak
                ...
                <div class="tab-content" id="myTabContent">
                    <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                        @if($jenis=='billing')
                        @include('adminDesa.akunPajak.billingUpdate')
                        @elseif($jenis=='buku')
                        @include('adminDesa.akunPajak.bukuPajakUpdate')
                        @endif
...
<script>
    ...
    var totalPajak = $('#totalPajak').val();
          totalPajak = Number(totalPajak);
    var pajakTerbayar = $('#pajakTerbayar').val();
        pajakTerbayar = Number(pajakTerbayar);
    $('#pajakBelum').val(totalPajak-pajakTerbayar);
    var progress = (pajakTerbayar/totalPajak) * 100;
        progress = progress.toFixed(2);
    $('#progressPajak').val(progress);
...
$(".buku_pembantu_pajak").change(function(event) {
    var nfile = $(this).val();
    getURLa(this, nfile);
});
...
if (input.files && input.files[0]) {
    ...
    if (cekgb == 'pdf' || cekgb == 'PDF') {
        if(input.files[0]['size'] > 5120000){
            ...
            $('.buku_pembantu_pajak').val("");
            $('.label_buku_pembantu_pajak').html("Choose file PDF (max: 5 MB)");
        }else{
